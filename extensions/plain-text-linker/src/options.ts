...
function provideNotifyInvalidUrl(settings: Settings): void {
    const notifyInvalidUrlCheckbox = document.getElementById(
        "notifyInvalidUrl"
    ) as HTMLInputElement;
...
function provideTryHttp(settings: Settings): void {
    const tryHttpCheckbox = document.getElementById(
        "tryHttp"
    ) as HTMLInputElement;
...
function provideForegroundByDefault(settings: Settings): void {
    const foregroundByDefault = document.getElementById(
        "foregroundByDefault"
    ) as HTMLInputElement;
...
function provideNewTabPosition(settings: Settings): void {
    const newTabPositionPickerContainer = document.getElementById(
        "newTabPositionPicker"
    ) as HTMLFieldSetElement;
    const [initialPosition] = Object.values(Position).filter(
        (key) => key === settings.newTabPosition
    );
    document.querySelector<HTMLInputElement>(
        `#newTabPositionPicker input[value='${initialPosition}']`
    )!.checked = true;

    newTabPositionPickerContainer.addEventListener("change", (event) => {
        const newTabPosition = (event.target as HTMLInputElement)
            .value as Position;
