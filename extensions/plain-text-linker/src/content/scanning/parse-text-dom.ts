...
function adder<T>(direction: "up" | "down", array: T[]): (value: T) => void {
    return direction === "up" ? text => array.unshift(text) : text => array.push(text);
}

export function step(direction: "up" | "down", result: string[]): (node: Node, index: number) => boolean {
    const substring: (text: string, index: number) => string =
        direction === "up" ? (text, index) => text.substring(0, index) : (text, index) => text.substring(index, Infinity);
...
export function walk(direction: "up" | "down", result: string[]): (pivot: Node) => void {
    ...
    return pivot => {
        for (const node of walkTextNodes(pivot)) {
            if (node.textContent == null || ONLY_WHITESPACE.test(node.textContent)) {
