...
function fetcher(id: IpService, url: URL, extract: (response: string) => string): GetIp {
    return async () => {
...
export function ipGetter(service: IpService): [GetIp, number] {
    switch (service) {
        case IpService.curl:
            return [fetcher(IpService.curl, new URL("https://curlmyip.net/"), trim), 1];

        case IpService.iCanHaz:
            return [fetcher(IpService.iCanHaz, new URL("https://icanhazip.com/"), trim), 1];

        case IpService.json: {
            const instance = fetcher(IpService.json, new URL("https://jsonip.com/"), body => {
        ...
        case IpService.myExt:
            return [fetcher(IpService.myExt, new URL("https://myexternalip.com/raw"), trim), 1];

        case IpService.whatIs:
            return [fetcher(IpService.whatIs, new URL("https://bot.whatismyipaddress.com/"), trim), 5];

        case IpService.ifConfig:
            return [fetcher(IpService.ifConfig, new URL("https://ifconfig.co/ip"), trim), 1];
