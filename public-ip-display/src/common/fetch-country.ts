...
import {CountryData} from "./ipdata";
import {CountryService} from "./settings/enums";

type GetCountry = () => Promise<CountryData>;

function fetcher(id: CountryService, url: URL, extract: (response: string) => string): GetCountry {
    return async () => {
...
export function countryGetter(service: CountryService): [GetCountry, number] {
    switch (service) {
        case CountryService.ifConfig:
            return [fetcher(CountryService.ifConfig, new URL("https://ifconfig.co/country-iso"), body => body.trim()), 1];
        ...
        case CountryService.freeGeo: {
            ...
            return [fetcher(CountryService.freeGeo, new URL("https://freegeoip.app/json/"), extractFromJson), 1];
        }
        ...
        case CountryService.ipApi: {
            ...
            return [fetcher(CountryService.ipApi, new URL("https://ipapi.co/json/"), extractFromJson), 2];
        }
